...
const preProcessOptions = (options: Array<SelectFieldOption>): OptionProcessingResults => {
  ...
  if(groups.length > 1) {
    groups.forEach(group => {
      const groupOptions = groupedOptions.filter(option => option.groupName === group)
      processedOptions.push(...groupOptions);
    })

    const unGroupedOptions = options.filter(option => !Boolean(option.groupName));
